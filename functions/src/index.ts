...
const firebaseStorageBucket = defineSecret("NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET");
...
export const updateExchangeRate = onSchedule(
    ...
    async (event) => {
        try {
            logger.info("Starting scheduled exchange rate update", { structuredData: true });
            ...
            // Save to Firestore
            await db.collection("exchange_rate").doc(documentId).set(exchangeRateData);

            logger.info(`Exchange rates updated successfully with document ID: ${documentId}`, {
