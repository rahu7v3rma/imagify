...
export async function GET(request: NextRequest) {
  try {
    ...
    const subscriptionPlanCredits = SUBSCRIPTION_PLAN_CREDITS[subscriptionPlanId];
    ...
    return NextResponse.redirect(
      new URL("/dashboard/billing?tab=subscription&buy_subscription_success=1", request.url)
    );
  } catch (error: unknown) {
    if (IS_PRODUCTION) {
    ...
    return NextResponse.redirect(
      new URL("/dashboard/billing?tab=subscription&buy_subscription_failure=1", request.url)
    );
  }
}
