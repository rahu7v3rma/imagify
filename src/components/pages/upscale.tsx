...
export default function UpscalePage() {
  ...
  const { mutate: upscale, isPending: isUpscalePending } =
    trpc.upscale.upscale.useMutation({
      onSuccess: (data) => {
  ...
  const onSubmit = async (data: UpscaleFormValues) => {
    setSuccessMessage(null);
    setErrorMessage(null);
    upscale({
  ...
  const setFormValue = (field: keyof UpscaleFormValues, value: string) => {
              ...
              <CardHeader>
                <CardTitle>
                  <div className="flex flex-col items-start">
                    <H1>Upscale Image</H1>
                    <Muted>Upload an image to upscale it using AI</Muted>
                  </div>
                </CardTitle>
